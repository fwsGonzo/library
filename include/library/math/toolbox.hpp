...
{
	float a = (3.0f*x[1] - 3.0f*x[2] + x[3] - x[0]) * 0.5f;
	float b = (2.0f*x[0] - 5.0f*x[1] + 4.0f*x[2] - x[3]) * 0.5f;
	float c = (x[2] - x[0]) * 0.5f;
	float d = x[1];
...
{
    T arr[4] = {cubic_fast(p + 0, y), cubic_fast(p + 4, y), cubic_fast(p + 8, y),
                cubic_fast(p + 12, y)};
...
{
    T arr[4] = {catmull_rom(p + 0, y), catmull_rom(p + 4, y), catmull_rom(p + 8, y),
                catmull_rom(p + 12, y)};
...
{
    T arr[4] = {bicubic(p + 0, x, y), bicubic(p + 16, x, y), bicubic(p + 32, x, y),
                bicubic(p + 48, x, y)};
...
{
    return ((1 - z) * (1 - y) * (1 - x) * p[0] + (1 - z) * (1 - y) * (1 + x) * p[1] +
            (1 - z) * (1 + y) * (1 - x) * p[2] + (1 - z) * (1 + y) * (1 + x) * p[3] +
            (1 + z) * (1 - y) * (1 - x) * p[4] + (1 + z) * (1 - y) * (1 + x) * p[5] +
            (1 + z) * (1 + y) * (1 - x) * p[6] + (1 + z) * (1 + y) * (1 + x) * p[7]) /
           8.0f;
}
} // namespace library
