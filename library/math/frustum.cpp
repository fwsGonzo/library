...
void Frustum::normalizePlane(int side)
{
    ...
    float inv_mag =
        1.0 / sqrtf(frustum[side][A] * frustum[side][A] + frustum[side][B] * frustum[side][B] +
...
bool Frustum::cube(float x, float y, float z, float size) const
{
    ...
    for (int i = 0; i < 6; i++)
    {
        if (frustum[i][A] * (x - size) + frustum[i][B] * (y - size_y) + frustum[i][C] * (z - size) +
                frustum[i][D] >
            0)
            continue;
...
bool Frustum::column(float x, float z, int cy, int height, float size) const
{
    ...
    for (int i = 0; i < 6; i++)
    {
        if (frustum[i][A] * (x - size) + frustum[i][B] * (base_y) + frustum[i][C] * (z - size) +
                frustum[i][D] >
            0)
            continue;
        ...
        if (frustum[i][A] * (x - size) + frustum[i][B] * (size_y) + frustum[i][C] * (z - size) +
                frustum[i][D] >
            0)
            continue;
...
void Frustum::createPerspective(float fovy, float aspect, float znear, float zfar)
{
    ...
    frustum[2][0] = co;
    frustum[2][1] = 0.0f;
    frustum[2][2] = si * aspect;
    frustum[2][3] = 0.0f;
    frustum[3][0] = -co;
    frustum[3][1] = 0.0f;
    frustum[3][2] = si * aspect;
    frustum[3][3] = 0.0f;
    frustum[4][0] = 0.0f;
    frustum[4][1] = 0.0f;
    frustum[4][2] = 1.0f;
