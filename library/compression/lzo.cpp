...
LZO::LZO()
{
    ...
    {
        if (lzo_init() != LZO_E_OK) throw std::string("LZO::LZO() Failed to initialize lzo2");
    }
}
...
lzo_bytep LZO::getData() { return compression_buffer; }

lzo_uint LZO::getDataLength() { return compression_length; }
...
bool LZO::compress1x(lzo_bytep data, int datalen)
{
    ...
    return lzo1x_1_15_compress(data, datalen, compression_buffer, &compression_length,
                               lzo_workmem) == LZO_E_OK;
}
bool LZO::optimize1x(lzo_bytep data, int datalen)
{
    // optimizes a 1x compressed block
    return lzo1x_optimize(data, datalen, compression_buffer, &compression_length, lzo_workmem) ==
           LZO_E_OK;
}
bool LZO::decompress1x(lzo_bytep data, int datalen)
{
    ...
    return lzo1x_decompress(data, datalen, compression_buffer, &compression_length, nullptr) ==
           LZO_E_OK;
}
...
bool LZO::compress2a(lzo_bytep data, int datalen)
{
    // compress n (to n + nlen) into compression_buffer
    // lzobuffer.compression_length is the resulting compressed size
    return lzo2a_999_compress(data, datalen, compression_buffer, &compression_length,
                              lzo_workmem) == LZO_E_OK;
}
bool LZO::decompress2a(lzo_bytep data, int datalen)
{
    ...
    // lzobuffer.compression_length is the resulting uncompressed size
    return lzo2a_decompress(data, datalen, compression_buffer, &compression_length, nullptr) ==
           LZO_E_OK;
}
