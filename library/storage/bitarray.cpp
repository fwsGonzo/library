...
BitArray& BitArray::operator=(const BitArray& ba)
{
    size = ba.size;
    bits = std::unique_ptr<bitarray_t[]>(new bitarray_t[this->size]);
    if (size) { memcpy(bits.get(), ba.bits.get(), sizeof(bitarray_t) * size); }
    return *this;
}

BitArray::bitarray_t BitArray::segsize(int size) const { return (size >> SEG_SHR) + 1; }

void BitArray::clear() { memset(bits.get(), 0, sizeof(bitarray_t) * size); }
