...
Log::Log(std::string filename) : Log() { this->open(filename); }
...
void Log::setOutputConsole(bool conout) { this->outputConsole = conout; }
...
Log& operator<<(Log& out, const Log::LogLevel level)
{
    if (level) // not ENDL
    {
        ...
        if (t) { ss << "[" << t->tm_hour << ":" << t->tm_min << ":" << t->tm_sec << "] "; }
        out.log += ss.str();
    ...
    else // Log::log_level_t.ENDL
    {
        ...
        // output to file
        if (out.file.good()) out.file << out.log << endl;
        // clear log string
        out.log = "";
...
bool Log::write(LogLevel level, std::string text)
{
    ...
    ss << ": " << text << endl;
    // to file
    if (file.good()) file << ss.str();
    // output to console
    if (this->outputConsole) { cout << ss.str() << endl; }
