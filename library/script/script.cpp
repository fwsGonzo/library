...
Script::Script()
{
    ...
    if (this->state == nullptr) { throw std::string("Failed to initialize libtcc"); }
}

Script::~Script()
{
    if (this->state) tcc_delete(this->state);

    if (this->memory) free(this->memory);
}

void Script::setErrorFunc(error_func errfunc) { tcc_set_error_func(this->state, nullptr, errfunc); }
...
void Script::compile(const std::string& program)
{
    ...
    if (tcc_relocate(this->state, this->memory) == TCC_ERROR) { throw std::string("Link error"); }
}
