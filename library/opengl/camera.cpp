...
void Camera::calculateFrustum() { frustum.calculate(matviewproj); }
const Frustum& Camera::getFrustum() const { return this->frustum; }

void Camera::setRotation(float rx, float ry, float rz) { matrot = rotationMatrix(rx, ry, rz); }
...
void Camera::setRotationMatrix(const glm::mat4& rot) { matrot = rot; }
