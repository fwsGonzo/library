...
void Texture::create(const Bitmap& bmp, bool mipmap = true, GLint wrapmode = GL_CLAMP_TO_EDGE,
                     GLint magfilter = GL_NEAREST, GLint minfilter = GL_LINEAR_MIPMAP_LINEAR)
{
    ...
    else if (this->type == GL_TEXTURE_CUBE_MAP)
    {
        ...
        for (int i = 0; i < 6; i++)
        {
            ...
            glTexImage2D(GL_TEXTURE_CUBE_MAP_POSITIVE_X + i, 0, format, cmsize, cmsize, 0,
                         bmp.getFormat(), byteFormat, blitdump.data());
        }
        /* ====================================== */
    }
    else if (this->type == GL_TEXTURE_2D_ARRAY)
    {
        ...
        glTexImage3D(this->type, 0, format, width, height, numTiles, 0, bmp.getFormat(), byteFormat,
                     pixel);
    }
    else
    {
        logger << Log::ERR << "@Texture::create(Bitmap&): Unknown texture target ("
               << (int) this->type << ")" << Log::ENDL;
        throw std::runtime_error("Texture::create(Bitmap&): Unknown texture target (" +
                                 std::to_string(this->type) + ")");
    }

    if (this->isMipmapped) { glGenerateMipmap(this->type); }
...
void Texture::create(int levels, int width, int height)
{
    ...
    case GL_TEXTURE_2D:
        ...
        break;
    default:
        logger << Log::ERR << "Texture::create(): Unknown texture target (" << this->type << ")"
               << Log::ENDL;
...
void Texture::create3d(int levels, int x, int y, int z)
{
    ...
    case GL_TEXTURE_3D:
        ...
        break;
    default:
        logger << Log::ERR << "Texture::create3d(): Unknown texture target (" << this->type << ")"
               << Log::ENDL;
        logger << Log::ERR << toString();
...
void Texture::createDepth(int width, int height)
{
    ...
    if (format == GL_DEPTH24_STENCIL8 || format == GL_DEPTH32F_STENCIL8)
    {
        ...
        glTexImage2D(this->type, 0, format, width, height, 0, GL_DEPTH_STENCIL,
                     GL_UNSIGNED_INT_24_8, nullptr);
    }
    else
    {
        glTexImage2D(this->type, 0, format, width, height, 0, GL_DEPTH_COMPONENT, GL_FLOAT,
                     nullptr);
    }
    if (OpenGL::checkError())
    {
...
void Texture::copyScreen() { copyScreen(this->width, this->height); }
...
void Texture::upload(const Bitmap& bmp)
{
    ...
    {
        glTexImage2D(this->type, 0, format, width, height, 0, bmp.getFormat(), getByteFormat(),
                     bmp.data());
    }
...
GLenum Texture::getStorageFormat()
{
    if (format == GL_RGBA16F || format == GL_R16F || format == GL_RGBA32F || format == GL_R32F)
        ...
        return GL_RGBA_INTEGER;
    else if (format == GL_RGBA16UI || format == GL_RGB16UI || format == GL_RG16UI ||
             format == GL_R16UI)
...
GLenum Texture::getByteFormat()
{
    if (format == GL_RGBA16F || format == GL_R16F || format == GL_RGBA32F || format == GL_R32F)
        return GL_FLOAT;
    else if (format == GL_RGBA32UI || format == GL_RGB32UI || format == GL_RG32UI ||
             format == GL_R32UI)
...
std::string Texture::toString() const
{
    ...
    {
    case GL_TEXTURE_1D:
        typeString = "GL_TEXTURE_1D";
        break;
    case GL_TEXTURE_2D:
        typeString = "GL_TEXTURE_2D";
        break;
    case GL_TEXTURE_2D_MULTISAMPLE:
        typeString = "GL_TEXTURE_2D_MULTISAMPLE";
        break;
    case GL_TEXTURE_2D_MULTISAMPLE_ARRAY:
        typeString = "GL_TEXTURE_2D_MULTISAMPLE_ARRAY";
        break;
    case GL_TEXTURE_2D_ARRAY:
        typeString = "GL_TEXTURE_2D_ARRAY";
        break;
    case GL_TEXTURE_3D:
        typeString = "GL_TEXTURE_3D";
        break;
    case GL_TEXTURE_CUBE_MAP:
        typeString = "GL_TEXTURE_CUBE_MAP";
        break;
    default:
    ...
    case GL_RGBA:
        ...
        break;
    case GL_RGBA16F_ARB:
        formatString = "GL_RGBA16F";
        break;
    case GL_RGBA32F_ARB:
        formatString = "GL_RGBA32F";
        break;
    case GL_COMPRESSED_RGBA:
        formatString = "GL_COMPRESSED_RGBA";
        break;
    case GL_DEPTH_COMPONENT:
        formatString = "GL_DEPTH_COMPONENT";
        break;
    case GL_DEPTH_COMPONENT16:
        formatString = "GL_DEPTH_COMPONENT16";
        break;
    case GL_DEPTH_COMPONENT24:
        formatString = "GL_DEPTH_COMPONENT24";
        break;
    case GL_DEPTH_COMPONENT32:
        formatString = "GL_DEPTH_COMPONENT32";
        break;
    case GL_DEPTH24_STENCIL8:
        formatString = "GL_DEPTH24_STENCIL8";
        break;
    default:
    ...
    std::stringstream ss;
    ss << "Texture ID: " << id << " Size: (" << width << ", " << height
